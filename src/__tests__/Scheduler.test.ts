...
function makeDate(isoDay: number, hours: number, minutes: number): Date {
...
describe('Scheduler', () => {
  ...
  beforeEach(() => {
    jest.useFakeTimers();
    onChange = jest.fn();
    scheduler = new Scheduler(onChange);
  });
  ...
  describe('state transitions', () => {
    it('does not re-fire if state does not change between ticks', () => {
      ...
      scheduler.setSchedule(['1-5 09:00-17:00']);
      scheduler.start();
      jest.advanceTimersByTime(30_000); // one tick
      jest.advanceTimersByTime(30_000); // second tick
