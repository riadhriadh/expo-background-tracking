import { haversine, pointInPolygon, centroid, uuidv4 } from '../geo';
...
describe('haversine', () => {
  ...
  it('calculates Paris → London (~341 km) within 1 km', () => {
    const dist = haversine(48.8566, 2.3522, 51.5074, -0.1278);
  ...
  it('calculates a short distance (~111 m per 0.001°) correctly', () => {
    ...
    const dist = haversine(48.0, 2.0, 48.001, 2.0);
    expect(dist).toBeGreaterThan(100);
    expect(dist).toBeLessThan(120);
  });

  it('is symmetric (A→B === B→A)', () => {
    const ab = haversine(48.0, 2.0, 49.0, 3.0);
    const ba = haversine(49.0, 3.0, 48.0, 2.0);
...
describe('pointInPolygon', () => {
  ...
  it('handles a triangle', () => {
    ...
    const triangle: Array<[number, number]> = [[0, 0.5], [1, 0], [1, 1]];
    expect(pointInPolygon(0.8, 0.5, triangle)).toBe(true);   // deep inside
    expect(pointInPolygon(0.1, 0.1, triangle)).toBe(false);  // left of left edge at lat=0.1
  });

  it('handles a real-world bounding box (Paris)', () => {
    const paris: Array<[number, number]> = [
      [48.815, 2.224], [48.815, 2.470],
      [48.902, 2.470], [48.902, 2.224],
    ];
    expect(pointInPolygon(48.8566, 2.3522, paris)).toBe(true); // Eiffel Tower
    expect(pointInPolygon(51.5074, -0.1278, paris)).toBe(false); // London
  });
});

describe('centroid', () => {
  it('returns the midpoint for two vertices', () => {
    const result = centroid([[0, 0], [2, 4]]);
    expect(result.latitude).toBeCloseTo(1);
    expect(result.longitude).toBeCloseTo(2);
  });

  it('returns the single vertex for one vertex', () => {
    const result = centroid([[48.8566, 2.3522]]);
  ...
  it('returns the geometric center of a square', () => {
    const result = centroid([[0, 0], [0, 2], [2, 2], [2, 0]]);
    expect(result.latitude).toBeCloseTo(1);
